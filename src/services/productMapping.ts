 ...
 */
export async function getValidatedProductId(originalId: number): Promise<string | null> {
  const databaseId = getProductDatabaseId(originalId);
  
  if (!databaseId) {
    console.error(`❌ No database ID found for original product ID: ${originalId}`);
    return null;
  }
  ...
  if (!exists) {
    console.error(`❌ Product with database ID ${databaseId} does not exist in database`);
    return null;
  }
