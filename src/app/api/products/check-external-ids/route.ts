...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: productsWithoutExternalId, error: noExternalIdError } =
      await supabaseAdmin
        .from("products")
    ...
    if (noExternalIdError) {
      console.error(
        "❌ Error checking products without external_id:",
        noExternalIdError
      );
      return NextResponse.json({
        success: false,
        error: noExternalIdError.message,
      });
    }

    // Перевіряємо товари з порожнім external_id
    const { data: productsWithEmptyExternalId, error: emptyExternalIdError } =
      await supabaseAdmin
        .from("products")
        .select("id, external_id, name, category_id")
        .eq("external_id", "");
    ...
    if (emptyExternalIdError) {
      console.error(
        "❌ Error checking products with empty external_id:",
        emptyExternalIdError
      );
      return NextResponse.json({
        success: false,
        error: emptyExternalIdError.message,
      });
    }
    ...
    console.log(
      `📊 Found ${
        productsWithoutExternalId?.length || 0
      } products without external_id`
    );
    console.log(
      `📊 Found ${
        productsWithEmptyExternalId?.length || 0
      } products with empty external_id`
    );
    ...
    return NextResponse.json({
      ...
      productsWithEmptyExternalId:
        productsWithEmptyExternalId?.slice(0, 10) || [],
    });
  } catch (error) {
