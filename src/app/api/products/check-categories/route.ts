...
export async function GET(request: NextRequest) {
  try {
    ...
    if (allProductsError) {
      ...
      return NextResponse.json({ success: false, error: allProductsError.message });
    }
    ...
    if (categoriesError) {
      ...
      return NextResponse.json({ success: false, error: categoriesError.message });
    }
    ...
    console.log(`📊 Found ${productsWithoutCategories?.length || 0} products without categories`);
    console.log(`📊 Found ${productsWithInvalidCategories.length} products with invalid categories`);
    ...
    return NextResponse.json({
      ...
      stats: {
        ...
        withValidCategories: (allProducts?.length || 0) - (productsWithoutCategories?.length || 0) - productsWithInvalidCategories.length
      },
