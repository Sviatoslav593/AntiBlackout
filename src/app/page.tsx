...
function HomeContent() {
    ...
    async (
      ...
      try {
        ...
        if (filterParams?.minCapacity)
          params.append("minCapacity", filterParams.minCapacity.toString());
        if (filterParams?.maxCapacity)
          params.append("maxCapacity", filterParams.maxCapacity.toString());
  ...
  useEffect(() => {
    ...
    const applyFilters = async () => {
      ...
      // Add capacity filters for power banks
      if (filters.capacityRange.min > 0) {
        filterParams.minCapacity = filters.capacityRange.min;
        console.log("Added minCapacity filter:", filters.capacityRange.min);
      }
      ...
      // Add USB filters
      if (filters.usbFilters?.inputConnector) {
        filterParams.inputConnector = filters.usbFilters.inputConnector;
      }
      if (filters.usbFilters?.outputConnector) {
        filterParams.outputConnector = filters.usbFilters.outputConnector;
      }
      if (filters.usbFilters?.cableLength) {
      ...
      console.log("Filter params details:", {
        ...
        categories: filterParams.categoryIds,
        brands: filterParams.brandIds,
      });
      setCurrentPage(1);
            ...
            <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
              ...
              <Filters
                ...
                availableCategories={availableCategories}
                  ...
                  selectedCategoryId={(() => {
                    const categoryId = filters.categories.includes("Портативні батареї") ? 1001 :
                      filters.categories.includes("Зарядки та кабелі") ? 1002 :
                      undefined;
